...
To try it without waiting a day, staging exposes `POST /cvc-recollection/expire` with `{ paymentMethodId }`: it ages the card's CVC clock out on the spot. In Step 3, select `encrypted-card` and click **Simulate CVC expiry**; the app calls it, re-reads the allowance and shows the CVC form. To see the mint-time `409` instead, run it from a second tab and click **Reveal and decrypt card** in the first. The control and the endpoint do not exist in production.

`pnpm test` runs the unit tests for the 409 recovery path and the timeline explanations.

When you select `encrypted-card` in Step 3, choose **Generate a temporary key** and click **Reveal and decrypt card** to generate a keypair and decrypt locally in one action. For your own RSA 2048 public key (PEM/SPKI), choose **Use my own public key** and **Get encrypted card**; copy the JWE or decrypt it in this browser with the matching private key (PEM/PKCS#8). Private keys remain in this tab. The staging-only simulation control sits beside the reveal action.
